...
skippy_run_init(MainWin *mw, Window leader) {
	...
	// fputs("\n");
	// printfef("(): mw->clients = do_layout(mw, mw->clients, mw->revert_focus_win, leader);");
			...
			if (0 == read_ret) {
				...
				switch (piped_input) {
					case PIPECMD_QUEUE_FI_PREV:
						...
						// printfdf("(): Received focus initial: %i", ps->o.focus_initial);
						// usleep(10000); // we must pause slightly, otherwise will miss next read() call in this loop()
						break;
